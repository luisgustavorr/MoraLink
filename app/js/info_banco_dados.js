export class infoDBPage {
    ...
    submitAtChange() {
        ...
        $("#content_inf_db input, #content_inf_db textarea").change(
            async function (e) {
    ...
    submitAtEnter() {
        $("#content_inf_db input,#content_inf_db textarea").keyup(function (e) {
            if (e.keyCode == 13) {
                $(this).change()
            }
        })
    ...
    setValues(){
        $("#content_inf_db input, #content_inf_db textarea").each( async function(){
            let id = $(this).attr("id")
            console.log(id)
            let getVariable = await window.indexBridge.getVariable(id)
            console.log(getVariable)
